...
# 推理参数
HEF_PATH = "/home/firefly/Denoising-rk3588J/models/dncnn_identical/dncnn_identical.hef"  # Hailo模型路径
...
# 视频保存参数（新增）
SAVE_VIDEO_DIR = "output/inference_videos_identical"  # 视频保存目录
...
def stitch_frames(original_frame, infer_frame, fps):
    ...
    text_size = cv2.getTextSize(fps_text, TEXT_FONT, TEXT_SIZE, TEXT_THICKNESS)[0]
    text_x = stitched_frame.shape[1] - text_size[0] - 20  # 右偏移20
    text_y = stitched_frame.shape[0] - text_size[1] - 20  # 下偏移20
    cv2.putText(
        stitched_frame, 
        fps_text, 
        (text_x, text_y), 
...
def main():
    ...
    elif saved_total_frames < infer_total_frames * 0.9:
        print(f"   ⚠️  保存帧数（{saved_total_frames}）低于推理帧数（{infer_total_frames}），视频写入为瓶颈")
    else:
        print(f"   ✅ 性能匹配：读取→推理→保存全链路流畅，无明显瓶颈")

    total_time = time.time() - total_start_time
    print(f"\n5. 整体耗时: {total_time:.2f} 秒")
    print("="*60)
